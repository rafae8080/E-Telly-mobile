...
class _MyRequestsScreenState extends State<MyRequestsScreen>
  ...
  Future<void> _fetchRequests() async {
    ...
    try {
      final response =
          await _api.authenticatedGet('/api/community/requests/mine');
      if (response.statusCode == 200) {
        ...
        if (mounted)
          setState(() {
            _requests = List<Map<String, dynamic>>.from(list);
            _loading = false;
          });
      } else {
        if (mounted)
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : _error != null
              ...
              : TabBarView(
                  controller: _tabController,
                  children: ['open', 'matched', 'fulfilled', 'cancelled']
                      .map(
                        (tab) => _RequestList(
                            requests: _byStatus(tab),
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, i) {
          ...
          final activePledgeCount =
              pledges.where((p) => p['status'] != 'withdrawn').length;
          return Card(
              ...
              subtitle: Column(
                ...
                children: [
                  ...
                  if (activePledgeCount > 0)
                    Row(
                      children: [
                        const Icon(Icons.people, size: 12, color: ET_RED),
              ...
              onTap: () => Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) => MyRequestDetailScreen(
                      requestId: req['_id'] ?? '', initialData: req),
                ),
              ).then((_) => onRefresh()),
            ),
          );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final list = (body is Map)
            ? (body['requests'] as List? ?? [])
            : (body is List ? body : []);
        final found = list.firstWhere((r) => r['_id'] == widget.requestId,
            orElse: () => null);
        if (found != null && mounted) {
          setState(() {
            _request = Map<String, dynamic>.from(found);
          });
        }
      }
    } catch (_) {}
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Helper selected! Chat is now open.'),
        ...
        _showAlert(
            'Error', 'Failed to accept pledge (${response.statusCode}).');
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Match released. Your request is open again.'),
                backgroundColor: ET_ORANGE),
          );
          await _fetchDetail();
        }
      } else if (response.statusCode == 400) {
        _showAlert('Cannot Release',
            'This match can no longer be released — the helper may have already marked it delivered. Please contact CDRRMO.');
      } else {
        print('>>> releaseMatch ${response.statusCode}: ${response.body}');
        _showAlert(
            'Error', 'Failed to release match (${response.statusCode}).');
      }
    } catch (e) {
      _showAlert('Error', 'Error: $e');
    } finally {
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Expanded(
                              child: Text(pledge['name'] ?? 'Anonymous',
                                  style: const TextStyle(
                                      fontWeight: FontWeight.bold)),
                            ),
                            _Badge(
